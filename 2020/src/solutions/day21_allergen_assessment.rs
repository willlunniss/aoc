...
pub fn gen(input: &str) -> (HashMap<&str, usize>, HashMap<&str, Vec<HashSet<&str>>>) {
    ...
    for product in input.lines() {
        ...
        let (ingredients_str, allergens) = product.splitn(2, " (contains ").collect_tuple().unwrap();
        ...
        for allergen in allergens[..allergens.len()-1].split(", ") {
            allergen_to_ingredients.entry(allergen).or_default().push(ingredients.clone());
        }
...
fn analyse_ingredients(input: &str) -> (HashMap<&str, usize>, HashMap<&str, &str>) {
    ...
    loop {
        ...
        for (allergen, potential_ingredients) in allergen_to_ingredients.iter() {
            let minimal : HashSet<_> = potential_ingredients[0].iter().filter(|k| potential_ingredients.iter().skip(1).all(|s| s.contains(*k))).collect();
            if minimal.len() == 1 {
